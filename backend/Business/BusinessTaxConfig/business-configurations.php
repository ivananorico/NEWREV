...
function getBusinessConfigurations() {
    ...
    try {
        $stmt = $pdo->prepare("
            ...
            ORDER BY business_type, min_range
        ");
...
function createBusinessConfiguration() {
    ...
    $required = ['business_type', 'tax_base', 'first_year_base', 'min_range', 'max_range', 'tax_rate', 'effective_date'];
    ...
    try {
        $stmt = $pdo->prepare("
            INSERT INTO business_tax_rate_config (
                business_type, tax_base, first_year_base, min_range, max_range, 
                tax_rate, effective_date, expiration_date, remarks
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ");
    ...
    try {
        $stmt = $pdo->prepare("
            UPDATE business_tax_rate_config SET 
                business_type = ?, tax_base = ?, first_year_base = ?, 
                min_range = ?, max_range = ?, tax_rate = ?, 
...
function patchBusinessConfiguration() {
    ...
    $allowedFields = ['expiration_date', 'tax_rate', 'min_range', 'max_range'];
